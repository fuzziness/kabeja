...
public class HatchLineIterator implements Iterator {
	...
	public HatchLineIterator(DXFHatch hatch, HatchLineFamily pattern) {
		...
		Point end = new Point(this.hatchBounds.getMinimumX(), this.hatchBounds
				.getMinimumY(), 0);
		this.bounderyEdges.add(new ParametricLine(start, MathUtils.getVector(
				start, end)));

		// edge 1
		start = new Point(this.hatchBounds.getMinimumX(), this.hatchBounds
				.getMinimumY(), 0);
		end = new Point(this.hatchBounds.getMaximumX(), this.hatchBounds
				.getMinimumY(), 0);
		this.bounderyEdges.add(new ParametricLine(start, MathUtils.getVector(
				start, end)));

		// edge 2
		start = new Point(this.hatchBounds.getMaximumX(), this.hatchBounds
				.getMinimumY(), 0);
		end = new Point(this.hatchBounds.getMaximumX(), this.hatchBounds
				.getMaximumY(), 0);
		this.bounderyEdges.add(new ParametricLine(start, MathUtils.getVector(
		...
		start = new Point(this.hatchBounds.getMaximumX(), this.hatchBounds
				.getMaximumY(), 0);
		end = new Point(this.hatchBounds.getMinimumX(), this.hatchBounds
				.getMaximumY(), 0);
		this.bounderyEdges.add(new ParametricLine(start, MathUtils.getVector(
				start, end)));
	...
	protected double[] getRasterValues(double x, double y) {
		...
		if (this.r.getX() == 0.0) {
			...
			v[1] = (y - this.pattern.getBaseY() - this.v.getY() * v[0])
					/ this.r.getY();
			...
			v[1] = (x - this.pattern.getBaseX()) / this.r.getX();
			v[0] = (y - this.pattern.getBaseY() - this.r.getY() * v[1])
					/ this.v.getY();

		} else if (this.v.getY() == 0.0) {
			v[1] = (y - this.pattern.getBaseY()) / this.r.getY();
			v[0] = (x - this.pattern.getBaseX() - this.r.getX() * v[1])
					/ this.v.getX();
			...
			v[0] = (y - this.pattern.getBaseY() - x * a + this.pattern
					.getBaseX()
					* a)
					/ (this.v.getY() - (a * this.v.getX()));
			v[1] = (x - this.pattern.getBaseX() - this.v.getX() * v[0])
					/ this.r.getX();
