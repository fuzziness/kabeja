...
public class DXFPolyline extends DXFEntity implements SVGPathBoundaryElement {
    ...
    protected void addToBounds(DXFVertex start, DXFVertex end, Bounds bounds) {
        if (start.getBulge() != 0) {
            ...
            double r = getRadius(start.getBulge(), l);
            ...
            Vector d = getExtrusion().getNormal();
            d = MathUtils.crossProduct(edgeDirection, d);
            ...
            if (start.getBulge() > 0) {
                ...
                endQ = MathUtils.getQuadrant(end.getPoint(), mp);
                startQ = MathUtils.getQuadrant(start.getPoint(), mp);

            } else {
                mp = MathUtils.getPointOfStraightLine(mp, d, (-1 * t));
                startQ = MathUtils.getQuadrant(end.getPoint(), mp);
                endQ = MathUtils.getQuadrant(start.getPoint(), mp);
            ...
            while (endQ > startQ) {
                switch (startQ) {
                case 0:
                    bounds.addToBounds(mp.getX(), mp.getY() + r);
                    break;
                case 1:
                    bounds.addToBounds(mp.getX() - r, mp.getY());
                    break;
                case 2:
                    bounds.addToBounds(mp.getX(), mp.getY() - r);
                    break;
        ...
        bounds.addToBounds(end.getBounds());
